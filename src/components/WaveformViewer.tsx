...
import type { AudioData } from './features/waveform-viewer/types';
...
export function WaveformViewer() {
  ...
  const calculateCanvasWidthMemo = useCallback((duration: number): number => {
              ...
              {pendingTagStart !== null && (
                <div className="flex items-center justify-between p-2 bg-primary/10 rounded-md border border-primary/30">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => dispatch({ type: 'CLEAR_PENDING_TAG' })}
                    className="h-6 w-6 p-0 text-muted-foreground hover:text-destructive"
                  >
                    ×
                  </Button>
